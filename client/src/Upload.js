...
function Upload() {
  ...
  const getFileContext = (e) => {
    ...
    axios
      .get("http://api:4000/upload/status", {
        headers: {
          "x-file-name": fileId,
  ...
  const fileUpload = (totalChunksUploaded) => {
    ...
    if (totalChunksUploaded <= totalChunks) {
      ...
      axios
        .post("http://api:4000/upload/complete", {
          headers: {
            "x-file-name": fileId,
          },
        })
    ...
    var result2 = md5(fileId)
    if (window.result === result2) {
      console.log("BUNLAR EŞLEŞTİ")
    }
  };
  ...
  const uploadChunk = (chunk) => {
    ...
    axios
      .post("http://api:4000/upload/files", chunk, {
        headers: {
          "x-file-name": fileId,
